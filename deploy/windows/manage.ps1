...
$RequiredVariables = @(
    ...
    "HfHubOffline", "TransformersOffline"
)
...
function Test-GpuRuntime {
    Require-Image
    $PythonCode = "import torch, torchaudio, muq; assert torch.cuda.is_available(); print({'torch': torch.__version__, 'cuda_runtime': torch.version.cuda, 'gpu': torch.cuda.get_device_name(0), 'torchaudio': torchaudio.__version__, 'muq': 'ok'})"
    & docker run --rm --gpus $GpuDevices --entrypoint python $ImageName -c $PythonCode
    Assert-LastExitCode "CUDA runtime check"
}
